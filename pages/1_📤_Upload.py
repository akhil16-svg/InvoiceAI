...
# Initialize database for logged-in user
from utils import ai_engine

def init_database_for_user(user_email):
...
db = init_database_for_user(st.session_state.user['email'])

st.set_page_config(
    page_title="Upload Invoices",
    page_icon="📤",
    layout="wide",
    ...
    with col_gear:
        with st.popover("⚙️", use_container_width=True):
            if st.session_state.get('logged_in', False) and st.session_state.get('user'):
                ...
                st.markdown("---")
                if st.button("🚪 Logout", use_container_width=True, key="gear_logout_upload"):
...
[data-testid="stSidebarNavToggler"]:hover {
    ...
    /* ADD THIS CSS to your main.py <style> block */
...
/* This adds the green animated top border, just like the feature-card */
.hero-section::before {
    content: '';
    position: absolute;
    top: 0;
...
from utils.ui import render_nav
render_nav()


# ----------- UPLOAD SECTION (PAGE CONTENT) -------------
...
st.markdown("""
    <div class="section-header">
        ...
        <p class="section-subtitle">
            Drag and drop your invoices or click to upload. Supports multiple files in JPG, PNG, and JPEG formats.
        </p>
    </div>
""", unsafe_allow_html=True)
...
with toggle_col1:
    auto_save = st.toggle("⚡ Auto-Save to Database after processing", value=True, help="Automatically save extracted invoice data to the database without clicking 'Save'.")
with toggle_col2:
    ...
    use_ai = st.toggle(
        ...
        help="Extract fields with Gemini vision + OCR for much higher accuracy. "
             "Requires GOOGLE_API_KEY in .env — falls back to the regex parser otherwise.",
    )

parser = InvoiceParser(use_llm=use_ai)
...
if uploaded_files:
            ...
            with col2:
                with st.spinner('🔄 Processing with AI...'):
                    try:
                        ...
                        processed_invoices.append(parsed_data)

                        st.markdown("#### 📊 Extracted Information")
                        st.caption(f"Parser: `{parsed_data.get('parsing_method', 'regex')}`")
                        
                        info_col1, info_col2 = st.columns(2)
                        with info_col1:
                            st.markdown(f"**Vendor:** {parsed_data.get('vendor_name', 'Unknown')}")
                            st.markdown(f"**Invoice #:** {parsed_data.get('invoice_number', 'N/A')}")
                        with info_col2:
                            st.markdown(f"**Date:** {parsed_data.get('date', 'N/A')}")
                            st.markdown(f"**Total:** {parsed_data.get('currency', '')} {(parsed_data.get('total_amount') or 0):,.2f}")
                        
                        if parsed_data.get('fraud_flags'):
                            st.error(f"⚠️ {len(parsed_data['fraud_flags'])} fraud indicator(s) detected")
                            for flag in parsed_data['fraud_flags']:
                                st.warning(f"• {flag.replace('_', ' ').title()}")
                        else:
                            st.success("✅ No fraud indicators - Invoice verified")
                        
                        st.markdown("---")
                        
                        if auto_save:
                            if db.save_invoice(parsed_data):
                                st.success("✅ Auto-Saved to DB")
                            else:
                                st.error("❌ Failed to auto-save")
                                
                            st.download_button(
                                "📥 Download JSON",
                                json.dumps(parsed_data, indent=2),
                                f"invoice_{parsed_data.get('invoice_number', idx)}.json",
                            ...
                            with action_col1:
                                if st.button("💾 Save to DB", key=f"save_{idx}", use_container_width=True):
                                    if db.save_invoice(parsed_data):
                                        st.success("Saved!")
    ...
    if processed_invoices and len(processed_invoices) > 1:
        st.markdown("---")
        ...
        with batch_col1:
            if st.button("💾 Save All to Database", use_container_width=True, type="primary"):
                success_count = sum(1 for inv in processed_invoices if db.save_invoice(inv))
                st.success(f"✅ Saved {success_count}/{len(processed_invoices)} invoices")
        
        with batch_col2:
            all_json = json.dumps(processed_invoices, indent=2)
            st.download_button(
                "📥 Download All (JSON)",
                all_json,
                "all_invoices.json",
