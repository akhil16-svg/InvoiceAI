    ...
    with col_gear:
        with st.popover("⚙️", use_container_width=True):
            if st.session_state.get('logged_in', False) and st.session_state.get('user'):
                ...
                st.markdown("---")
                if st.button("🚪 Logout", use_container_width=True, key="gear_logout_analytics"):
...
# ----------- Analytic Dashboard Section (PAGE CONTENT) -------------
# This is the code you CUT from your old main.py

# st.markdown('<a name="dashboard"></a>', unsafe_allow_html=True)
# st.markdown('<div class="section section-sage">', unsafe_allow_html=True)
st.markdown("""
    <div class="section-header">
        <h2 class="section-title">Sage Financial Analytics</h2>
        <p class="section-subtitle">
            Real-time insights into your Sage Ledger spending with interactive visualizations
        </p>
    </div>
""", unsafe_allow_html=True)
...
if not all_invoices:
    st.info("📊 Upload invoices to see your analytics dashboard")
else:
    # Key metrics in stat boxes
    col1, col2, col3, col4 = st.columns(4)
    
    total_spending = analytics.calculate_total_spending(all_invoices)
    avg_amount = analytics.calculate_average_invoice_amount(all_invoices)
    ...
    with col1:
        st.markdown(f"""
            <div class="stat-box">
                <span class="stat-number">{currency} {total_spending:,.0f}</span>
    ...
    with col2:
        st.markdown(f"""
            <div class="stat-box">
                <span class="stat-number">{len(all_invoices)}</span>
    ...
    with col3:
        st.markdown(f"""
            <div class="stat-box">
                <span class="stat-number">{currency} {avg_amount:,.0f}</span>
    ...
    with col4:
        st.markdown(f"""
            <div class="stat-box">
                <span class="stat-number">{fraud_stats['fraud_rate']*100:.1f}%</span>
    ...
    with col1:
        ...
        time_data = analytics.get_spending_over_time(all_invoices)
        if not time_data.empty:
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=time_data['date'],
                y=time_data['total_amount'],
                mode='lines+markers',
    ...
    with col2:
        ...
        vendor_data = analytics.get_spending_by_vendor(all_invoices).head(5)
        if not vendor_data.empty:
            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=vendor_data['total_spending'],
                y=vendor_data['vendor_name'],
                orientation='h',
                marker=dict(
                    color='rgba(255, 255, 255, 0.8)',
