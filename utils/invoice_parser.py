...
class InvoiceParser:
    ...
    def __init__(self, use_llm: bool = False, llm_api_key: Optional[str] = None):
        ...
        self.llm_api_key = llm_api_key  # kept for backwards compatibility; the
        # AI engine reads credentials from the environment
    ...
    def parse(
        ...
        With use_llm=True (and Google AI credentials configured) extraction is
        done by Gemini from the invoice image + OCR text; the regex parser
        remains the offline fallback and is also used to fill any fields the
        LLM returned as null.
        """
        from utils import ai_engine
        if self.use_llm and ai_engine.is_ai_available():
            return self._parse_with_llm(text, image_bytes, media_type)
    ...
    def _parse_with_llm(
        ...
        """Gemini-based extraction with regex fallback for missing fields."""
        from utils import ai_engine

        regex_parsed = self._parse_with_regex(text)
        try:
            llm_parsed = ai_engine.extract_invoice_fields(
                ocr_text=text, image_bytes=image_bytes, media_type=media_type
            )
        except Exception as e:
            print(f"LLM extraction failed, falling back to regex: {e}")
            return regex_parsed

        # LLM result wins; regex fills the gaps it left as null
        merged = dict(regex_parsed)
        for key, value in llm_parsed.items():
            if value is not None and value != []:
                merged[key] = value
        merged["raw_text"] = text
        merged["parsing_method"] = "gemini+regex"
        merged["fraud_flags"] = self._detect_fraud(merged, text)
        return merged
