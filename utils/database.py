...
class InvoiceDatabase:
    ...
    def _init_postgres(self):
        ...
        # Earlier Streamlit builds used a global unique invoice_number. The
        # full-stack app keeps each user's archive isolated, so duplicates are
        # only considered conflicts inside the same account.
        cursor.execute('ALTER TABLE invoices DROP CONSTRAINT IF EXISTS invoices_invoice_number_key')
        cursor.execute('''
            CREATE UNIQUE INDEX IF NOT EXISTS idx_pg_invoice_user_unique
            ON invoices(invoice_number, user_email)
        ''')
    ...
    def _save_to_postgres(self, invoice_data: Dict) -> bool:
        ...
        try:
            cursor.execute('''
                ...
                ON CONFLICT (invoice_number, user_email) DO UPDATE SET
    ...
    def _get_all_from_postgres(self) -> List[Dict]:
        ...
        try:
            ...
            for row in rows:
                raw_data = row[0]
                invoices.append(raw_data if isinstance(raw_data, dict) else json.loads(raw_data))
            return invoices
        except Exception as e:
